...
public class Plant2 : MonoBehaviour
{
    ...
    void Grow(List<CombineInstance> instances, Vector3 pos, Quaternion rot, Vector3 scale, int max, int num = 0)
    {
        ...
        { // temp scope
            Quaternion randRot = rot * Quaternion.Euler(spreadDegrees, Random.Range(-120, 120f), 0);
            Quaternion upRot = Quaternion.RotateTowards(rot, Quaternion.identity, 45);
            Quaternion newRot = Quaternion.Lerp(randRot, upRot, .55f);
        ...
        if (num > 1)
        {
            if (num % 2 == 1)
            {
                ...
                float degrees = Random.Range(-1, 2) * 150;

                Quaternion newRot = Quaternion.Euler(endPoint - pos) * Quaternion.Euler(90, 0, degrees);
