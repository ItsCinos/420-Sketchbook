...
public class Plant1 : MonoBehaviour
{
    ...
    void Grow(List<CombineInstance> instances, Vector3 pos, Quaternion rot, Vector3 scale, int max, int num = 0)
    {
        ...
        { // temp scope
            ...
            Quaternion upRot = Quaternion.RotateTowards(rot, Quaternion.identity, 40);
            Quaternion newRot = Quaternion.Lerp(randRot, upRot, .2f);
        ...
        if (num > 1)
        {
            if (num % 2 == 1)
            {
                ...
                float degrees = Random.Range(-1, 2) * 110;

                Quaternion newRot = Quaternion.LookRotation(endPoint + pos) * Quaternion.Euler(90, 0, degrees);
